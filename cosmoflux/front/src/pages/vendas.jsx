...
// ── WhatsApp / cobrança ──
const WPP_KEY = 'cf-wpp-template';
const WPP_DEFAULT = 'Oi {nome}! 😊 Passando para lembrar da sua parcela de {valor} que venceu em {vencimento} (em atraso há {dias} dias). Qualquer dúvida estou à disposição. Obrigada!';
const getWppTemplate = () => { try { return localStorage.getItem(WPP_KEY) || WPP_DEFAULT; } catch { return WPP_DEFAULT; } };

const soDigitos = (tel) => String(tel||'').replace(/\D/g,'');
const telParaWa = (tel) => {
  let d = soDigitos(tel);
  if (!d) return null;
  if (d.length <= 11 && !d.startsWith('55')) d = '55' + d; // adiciona DDI Brasil
  return d;
};
const preencheTemplate = (tpl, dados) =>
  tpl.replace(/\{nome\}/g, dados.nome||'')
     .replace(/\{valor\}/g, dados.valor||'')
     .replace(/\{vencimento\}/g, dados.vencimento||'')
     .replace(/\{dias\}/g, dados.dias!=null?dados.dias:'')
     .replace(/\{total\}/g, dados.total||'');

const abrirCobranca = (pedido, showToast) => {
  const wa = telParaWa(pedido.cliente_telefone);
  if (!wa) { showToast && showToast('Cliente sem telefone cadastrado','err'); return; }
  const parcelas = pedido.parcelas || [];
  const abertas = parcelas.filter(p=>!p.pago);
  const venc = abertas.find(p=>p.status==='vencido') || abertas[0];
  const saldo = abertas.reduce((a,p)=>a+(p.saldo_restante??p.valor),0);
  const msg = preencheTemplate(getWppTemplate(), {
    nome: (pedido.cliente||'').split(' ')[0],
    valor: fmtBRL(saldo),
    vencimento: venc?.vencimento || '—',
    dias: venc?.dias_atraso || 0,
...
export default function Vendas() {
  ...
  const [tplDraft, setTplDraft] = useState(getWppTemplate);
  ...
  const receberRapido = async (p, e) => {
    e?.stopPropagation();
    const abertas = (p.parcelas||[]).filter(x=>!x.pago).sort((a,b)=>a.numero-b.numero);
    if (abertas.length === 0) { showToast('Nenhuma parcela em aberto','err'); return; }
    setPagandoP('row-'+p.id);
    try {
      await api.patch(`/parcelas/${abertas[0].id}/pagar`, {});
      const restantes = abertas.length - 1;
      showToast(restantes>0 ? `Parcela recebida · ${restantes} restante(s)` : 'Venda quitada! 🎉');
      load();
  ...
  return (
    <div className="cf-vd-root" data-theme={theme}>
      ...
      <div className="cf-vendas">
        ...
        <div className="cf-vd-head">
          ...
          <div style={{display:'flex',gap:8}}>
            <button className="cf-btn" onClick={()=>{setTplDraft(getWppTemplate());setCfgModal(true);}} title="Configurar mensagem de cobrança">
              <Ic d={ICONS.whats} size={15}/> <span style={{marginLeft:2}}>Cobrança</span>
            </button>
        ...
        <div className="cf-vd-table">
          ...
          {loading ? (
            ...
            const emAberto = !canc && (sp==='em_aberto' || sp==='vencido');
            const parcAberta = (p.parcelas||[]).filter(x=>!x.pago);
            const maxAtraso = Math.max(0, ...parcAberta.map(x=>x.dias_atraso||0));
            const temTel = !!soDigitos(p.cliente_telefone);
            return (
              <div key={p.id} className={`cf-vd-row${canc?' canc':''}`} onClick={()=>setSelected(p)}>
                ...
                <div className="cf-vd-cli">
                  ...
                  <div className="cf-vd-cli-info">
                    ...
                    <div className="cf-vd-cli-pay">
                      {txt(p.modo_pagamento)||'—'}
                      {maxAtraso>0 && <span style={{color:'var(--crit)',fontWeight:700}}> · venceu há {maxAtraso}d</span>}
                    </div>
                  </div>
                </div>
                ...
                <div className="cf-vd-actions" onClick={e=>e.stopPropagation()}>
                  {emAberto && temTel && (
                    <button className="cf-vd-act-wpp" title="Cobrar no WhatsApp" onClick={()=>abrirCobranca(p, showToast)}>
                      <Ic d={ICONS.whats} size={15}/>
                    </button>
                  )}
                  {emAberto && (
                    <button className="cf-vd-baixa" disabled={pagandoP==='row-'+p.id} onClick={e=>receberRapido(p,e)} title="Receber próxima parcela">
                      {pagandoP==='row-'+p.id ? '...' : <><Ic d={ICONS.check} size={13}/> Receber</>}
                    </button>
                  )}
                  {podeEntregar && !emAberto && (
                  ...
                  <div style={{display:'flex',gap:8}}>
                    {parcelas.filter(pc=>!pc.pago).length>0 && (
                      <button className="cf-btn" style={{background:'color-mix(in oklab,var(--warn) 12%,transparent)',color:'var(--warn)',borderColor:'color-mix(in oklab,var(--warn) 30%,transparent)'}}
                        onClick={()=>{setAbatModal(p);setAbatValor('');}}>◑ Abater</button>
                    )}
                    {parcelas.filter(pc=>!pc.pago).length>0 && soDigitos(p.cliente_telefone) && (
                      <button className="cf-btn" style={{background:'color-mix(in oklab,#25d366 14%,transparent)',color:'#25d366',borderColor:'color-mix(in oklab,#25d366 32%,transparent)'}}
                        onClick={()=>abrirCobranca(p, showToast)}><Ic d={ICONS.whats} size={14}/> Cobrar</button>
                    )}
                  </div>
      ...
      {/* Modal config template WhatsApp */}
      {cfgModal && (
        <Portal theme={theme}>
        <div className="cf-vd-dov" style={{zIndex:1020}} onClick={e=>{if(e.target===e.currentTarget)setCfgModal(false);}}>
          <div className="cf-vd-dmodal" style={{maxWidth:520}}>
            <div className="cf-vd-dhd">
              <div>
                <div className="cf-vd-dtitle"><Ic d={ICONS.whats} size={17}/> Mensagem de cobrança</div>
                <div className="cf-vd-dsub">Personalize o texto enviado no WhatsApp</div>
              </div>
              <button className="cf-mclose" onClick={()=>setCfgModal(false)}>×</button>
            </div>
            <div className="cf-vd-dbody">
              <div className="cf-fld">
                <label className="cf-fld-lbl">Texto da mensagem</label>
                <textarea className="cf-inp" rows={5} value={tplDraft} onChange={e=>setTplDraft(e.target.value)}
                  style={{resize:'vertical',lineHeight:1.5,fontFamily:'var(--font-ui)'}}/>
              </div>
              <div style={{fontSize:11,color:'var(--text-muted)',fontFamily:'var(--font-mono)',lineHeight:1.7}}>
                Variáveis disponíveis (clique para inserir):
                <div style={{display:'flex',gap:6,flexWrap:'wrap',marginTop:8}}>
                  {['{nome}','{valor}','{vencimento}','{dias}','{total}'].map(v=>(
                    <button key={v} onClick={()=>setTplDraft(t=>t+' '+v)}
                      style={{padding:'4px 9px',borderRadius:6,border:'1px solid var(--brand-line)',background:'var(--brand-soft)',color:'var(--brand)',cursor:'pointer',fontFamily:'var(--font-mono)',fontSize:11,fontWeight:600}}>
                      {v}
                    </button>
                  ))}
                </div>
              ...
              <div style={{background:'var(--surface-2)',border:'1px solid var(--border)',borderRadius:10,padding:'12px 14px'}}>
                <div style={{fontSize:9,fontWeight:700,letterSpacing:'.1em',color:'var(--text-muted)',fontFamily:'var(--font-mono)',marginBottom:8}}>PRÉVIA</div>
                <div style={{fontSize:13,color:'var(--text-dim)',lineHeight:1.5}}>
                  {preencheTemplate(tplDraft, {nome:'Maria', valor:'R$ 144,90', vencimento:'10/07/2026', dias:5, total:'R$ 289,80'})}
                </div>
              </div>
            </div>
            <div className="cf-vd-dfoot" style={{justifyContent:'flex-end'}}>
              <button className="cf-btn cf-btn-ghost" onClick={()=>{ setTplDraft(WPP_DEFAULT); }}>Restaurar padrão</button>
              <button className="cf-btn cf-btn-primary" onClick={()=>{ try{localStorage.setItem(WPP_KEY,tplDraft);}catch{}; showToast('Mensagem salva'); setCfgModal(false); }}>
                Salvar
              </button>
            </div>
          </div>
