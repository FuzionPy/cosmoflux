...
export default function Lucros() {
  ...
  // Usa pixels como o Dashboard (max 110px)
  const MAX_PX = 110;
          ...
          <div className="card">
            ...
            <div className="chart-wrap">
              <div className="chart-bars">
                {mensal.map((m, i) => {
                  const pxReceita = maxReceita > 0 ? Math.max((m.receita / maxReceita) * MAX_PX, m.receita>0?3:0) : 0;
                  const pxLucro   = maxReceita > 0 ? Math.max((m.lucro   / maxReceita) * MAX_PX, m.lucro>0?3:0)   : 0;
                      ...
                      <div className="bar-stack">
                        {pxReceita > 0 && (
                          <div className="bar-seg"
                            style={{height:`${pxReceita}px`, background: isAtual ? 'rgba(0,153,255,.5)' : 'rgba(255,255,255,.08)'}}
                            data-tip={`Receita: ${fmtBRL(m.receita)}`}/>
                        )}
                        {pxLucro > 0 && (
                          <div className="bar-seg"
                            style={{height:`${pxLucro}px`, background: isAtual ? '#00d4aa' : 'rgba(0,212,170,.35)'}}
          ...
          <div className="tbl-wrap">
            {loading ? (
              ...
              <table className="tbl">
                <thead>
                  <tr>
                    <th>#</th><th>Produto</th><th className="r">Qtd vendida</th>
                    <th className="r">Receita</th><th className="r">Custo total</th>
                    <th className="r">Lucro</th><th>Margem</th><th>Participação</th>
                  </tr>
                </thead>
                <tbody>
                  {prodOrdenados.map((p, i) => (
                    <tr key={p.id}>
                      ...
                      <td><span className={`mgm-badge ${margemClass(p.margem)}`}>{fmtPct(p.margem)}</span></td>
                      <td>
